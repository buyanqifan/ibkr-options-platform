...
STRATEGY_INFO = {
    ...
    "version": "0.2.0",
    "description": "QC-aligned Binbin God backtester with a simplified wheel decision chain.",
    "universe": DEFAULT_STOCK_POOL,
    "selection_criteria": {
        "SP Risk": "Only portfolio margin, symbol assignment cap, and trade assignment cap size new puts",
        "SP Roll": "Short puts roll only after premium capture threshold with time remaining",
        "Covered Call": "Assigned stock sells calls with a cost floor and one relaxed delta retry",
        "Fail-Safe": "Assigned stock can force-exit after hold-time and drawdown thresholds",
    },
}
...
def _build_configuration_panel(defaults):
    return html.Div(
        [
            ...
            _section_card("SP Risk", _build_rows(SP_RISK_FIELDS, defaults), color="secondary"),
            _section_card("SP Roll", _build_rows(SP_ROLL_FIELDS, defaults), color="dark"),
...
@callback(
    ...
    State("bbg-max-assignment-risk-per-trade", "value"),
    State("bbg-max-new-puts-per-day", "value"),
    State("bbg-roll-threshold-pct", "value"),
    State("bbg-min-dte-for-roll", "value"),
    State("bbg-roll-target-dte-min", "value"),
    State("bbg-roll-target-dte-max", "value"),
    State("bbg-cc-below-cost-enabled", "value"),
    State("bbg-cc-target-delta", "value"),
    State("bbg-cc-target-dte-min", "value"),
    State("bbg-cc-target-dte-max", "value"),
    State("bbg-cc-max-discount-to-cost", "value"),
    State("bbg-assigned-stock-fail-safe-enabled", "value"),
    State("bbg-assigned-stock-min-days-held", "value"),
    State("bbg-assigned-stock-drawdown-pct", "value"),
    State("bbg-assigned-stock-force-exit-pct", "value"),
...
def run_binbin_backtest(
    ...
    max_new_puts_per_day,
    roll_threshold_pct,
    min_dte_for_roll,
    roll_target_dte_min,
    roll_target_dte_max,
    cc_below_cost_enabled,
    cc_target_delta,
    cc_target_dte_min,
    cc_target_dte_max,
    cc_max_discount_to_cost,
    ...
    params = build_binbin_backtest_params(
        {
            ...
            "max_assignment_risk_per_trade": max_assignment_risk_per_trade,
            "max_new_puts_per_day": max_new_puts_per_day,
            "roll_threshold_pct": roll_threshold_pct,
            "min_dte_for_roll": min_dte_for_roll,
            "roll_target_dte_min": roll_target_dte_min,
            "roll_target_dte_max": roll_target_dte_max,
            "cc_below_cost_enabled": cc_below_cost_enabled,
            "cc_target_delta": cc_target_delta,
            "cc_target_dte_min": cc_target_dte_min,
            "cc_target_dte_max": cc_target_dte_max,
            "cc_max_discount_to_cost": cc_max_discount_to_cost,
            "assigned_stock_fail_safe_enabled": assigned_stock_fail_safe_enabled,
            "assigned_stock_min_days_held": assigned_stock_min_days_held,
            "assigned_stock_drawdown_pct": assigned_stock_drawdown_pct,
            "assigned_stock_force_exit_pct": assigned_stock_force_exit_pct,
